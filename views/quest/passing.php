    ...
    <div class="quest-form-block">
        <ol class="progress-points">
            <?php for ($i = 1; $i <= $tasksCount; $i++) : ?>
                <?php if ($i < $task->number) : ?>
                    <li class="completed"><span>Задание <?= $i ?></span></li>
                <?php elseif ($i == $task->number) : ?>
                    <li class="active hovered"><span>Задание <?= $i ?></span></li>
                <?php else : ?>
                    <li><span>Задание <?= $i ?></span></li>
                <?php endif; ?>
            <?php endfor; ?>
        </ol>
        <div class="hints-selection">
            <?php foreach ($allHints as $hint) : ?>
                ...
                <a id="hint-link-<?= $hint->id ?>" class="hint-link <?= $class ?>" data-id="<?= $hint->id ?>" <?= ($class == 'not-available') ? "data-text='Эта подсказка пока недоступна'" : '' ?>>
