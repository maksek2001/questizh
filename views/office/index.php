        ...
        <div class="panel-info">
            ...
            <?php if ($passingResults) : ?>
                <?php foreach ($passingResults as $result) : ?>
                    <div class="passing-result <?= Html::encode($result['status']) ?>">
                        <div class="result-header">
                            <a class="quest-name" href="<?= Url::toRoute(['quest/index', 'id' => $result['quest_id']]) ?>">
                                <?= Html::encode($result['quest_name']) ?>
                            </a>
                            <span class="status <?= Html::encode($result['status']) ?>" data-text="<?= QuestPassingResult::STATUSES_MESSAGES[$result['status']]['message'] ?>">
                        ...
                        <div class="result-info mb-0 mt-2">
                            ...
                            <?php else : ?>
                                ...
                                <?= Html::encode($result['spented_time']) ?>
                            <?php endif; ?>
                        </div>
                    </div>
                <?php endforeach; ?>
